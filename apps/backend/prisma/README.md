...
이 폴더는 PostgreSQL 17 스키마와 샘플 데이터를 팀원끼리 공유하기 위한 기준이다.

## 핵심 원칙

- DB 구조는 `schema.prisma`, `models/*.prisma`, `migrations/`로 공유한다.
- 샘플 데이터는 `seed.ts`로 공유한다.
- DB 인스턴스 자체는 팀원이 각자 따로 사용한다.
- 각 팀원은 본인 `.env`의 `DATABASE_URL`에 개인 개발 DB를 연결한다.
- 개인 개발 DB는 PostgreSQL 17 호환이어야 하며 Supabase Postgres, AWS RDS PostgreSQL, 로컬 PostgreSQL, Docker PostgreSQL 중 하나를 권장한다.
- migration 실행 role에는 `pg_trgm` extension 생성 또는 사용 권한, `public` schema 객체 생성 권한, RLS 활성화 권한이 필요하다.
- 실제 개발 DB 데이터는 공유하지 않는다.
- migration 파일은 Git에 커밋한다.
- 개발/스테이징/운영 DB 운영 절차는 `OPERATIONS.md`를 따른다.
- `src/generated/prisma/`는 `prisma generate`로 생성하므로 커밋하지 않는다.
...
`corepack pnpm run setup`은 내부에서 `db:generate`만 실행하므로 PostgreSQL이 꺼져 있어도 통과해야 한다. 반대로 `db:deploy`, `db:migrate`, `db:seed`는 실제 DB 연결이 없으면 실패하는 것이 정상이다.

backend workspace에서 직접 실행해도 된다.
...
이 명령은 팀 공통 migration을 내 개인 DB에 적용하는 흐름이다. 다른 팀원의 DB에는 영향을 주지 않는다.

## 크롤러 JSON 적재

크롤러는 DB에 바로 쓰지 않고 표준 JSON을 먼저 만든다. 사람이 산출물을 확인한 뒤 아래 명령으로 형식 검증을 먼저 한다.

```bash
corepack pnpm run db:import:jobs --dry-run ../../docs/research/job-sites/saramin_sample_2026-05-14.json
```

개인 개발 DB의 `DATABASE_URL`이 설정된 상태에서 실제 적재를 실행한다.

```bash
corepack pnpm run db:import:jobs -- ../../docs/research/job-sites/saramin_sample_2026-05-14.json
```

Import는 `(source, sourceJobId)` 기준 upsert라 같은 원본 공고를 다시 넣어도 중복 행을 만들지 않는다. `source`와 `sourceJobId`는 수집 중복 방지와 추적을 위해 필수다.

## 개발 DB 초기화

주의: 현재 `.env`의 `DATABASE_URL`이 가리키는 DB 데이터가 삭제된다. Supabase, AWS RDS 같은 원격 DB를 쓰는 경우에도 개인 개발용 DB인지 먼저 확인한다. 공용 DB나 운영 DB에서는 실행하지 않는다.
