...
- `docs/work-log/WORK_LOG.md`의 Figma 요약을 Supabase 초기 적재, import 파이프라인, 사람인 import check, Git 정리 상태까지 포함하도록 최신화
- `corepack pnpm run worklog:export`로 5/14 export 텍스트를 확인하고 Figma bridge를 실행
- `corepack pnpm run figma:apply-log` 결과 `Figma WORK_LOG replaced.` 확인
