...
### Jobs Country And Region Filters

- Added backend `location` query support alongside existing `country` filtering for `GET /api/jobs`.
- Added country and region selects to `/jobs`, deriving filter options from loaded job data and sending `country`/`location` query params to the backend.
- Verification:
  - Added failing service tests first for fallback and database location filtering, then made them pass.
  - `corepack pnpm --filter @neet2work/backend run test -- src/services/job.service.test.ts` passed after sandbox-escalated rerun; initial run failed only on `node_modules/.vite-temp` permission.
  - `corepack pnpm --filter @neet2work/frontend lint` passed.
  - `corepack pnpm --filter @neet2work/backend lint` passed.
  - `corepack pnpm --filter @neet2work/frontend build` passed after sandbox-escalated rerun.
  - `corepack pnpm --filter @neet2work/backend build` passed after sandbox-escalated rerun.
  - Browser verified country `JP` plus region `Tokyo - 23 Wards` filtering rendered matching backend jobs without console errors.
