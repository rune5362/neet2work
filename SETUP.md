...
| 운영체제 | 문서 | 추천 방식 |
| --- | --- | --- |
| Windows | [WINDOWS_SETUP.md](./WINDOWS_SETUP.md) | PowerShell + winget 직접 설치, 필요 시 nvm-windows |
...
처음 세팅하는 팀원은 `nvm-windows`를 설치하지 않아도 됩니다. Windows 기본 경로는 `winget`으로 Node.js LTS를 직접 설치하는 방식입니다.

`nvm-windows`를 이미 사용 중이거나 여러 Node 버전을 바꿔 써야 한다면 아래 방식도 가능합니다.
