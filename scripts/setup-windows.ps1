...
Write-Step "Corepack 확인"
if (-not (Test-Command "corepack")) {
  Refresh-Path
}

if (-not (Test-Command "corepack")) {
  throw "Corepack 명령을 찾지 못했습니다. Node.js 24 LTS 설치 후 PowerShell을 새로 열고 다시 실행해 주세요."
}

Write-Step "Corepack pnpm 11 확인"
Write-Host "pnpm: $(& corepack pnpm --version)" -ForegroundColor Green
...
Write-Step "세팅 완료"
Write-Host "개발 서버 실행: corepack pnpm run dev" -ForegroundColor Green
Write-Host "Docker 실행: corepack pnpm run docker:up" -ForegroundColor Green
Write-Host "테스트 실행: corepack pnpm run test" -ForegroundColor Green
