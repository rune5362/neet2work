...
class ContractTest(unittest.TestCase):
    def test_classifies_career_stage_conservatively(self) -> None:
        ...
        self.assertEqual(classify_career_stage(posting("5", title="2026년 전산개발 모집공고", career_level="경력"))[0], "mid")
    ...
    def test_classifies_it_job_categories(self) -> None:
        ...
        self.assertEqual(classify_job_category("소프트웨어 개발 경력", [], "웹 플랫폼 개발")[0], "software_engineering")
        ...
        self.assertEqual(classify_job_category("Network Engineer", [], "TCP/IP and cloud operations")[0], "it_infrastructure_security")
    ...
    def test_classifies_non_it_as_excluded(self) -> None:
        ...
        self.assertEqual(classify_job_category("산업보안요원", [], "4조3교대 시설 보안 근무")[0], "non_it")
        self.assertEqual(classify_job_category("현대엔지니어링 재경본부 사업원가관리", [], "회계 결산")[0], "non_it")
        self.assertEqual(classify_job_category("반도체 CMP Slurry 소재 개발 담당자", [], "생산 제조 연구개발")[0], "non_it")
        self.assertEqual(classify_job_category("Regulatory Affairs Specialist", ["연구개발/설계"], "의료 인허가")[0], "non_it")
        self.assertEqual(classify_job_category("인플루언서마케터 체험형 인턴", ["마케팅/PR/분석"], "광고 홍보")[0], "non_it")
        self.assertEqual(
            classify_job_category(
                "Real estate agent for foreigners",
                ["Network", "Management", "Hospitality"],
                "Remote customer support for apartment search and real estate contracts.",
            )[0],
            "non_it",
        )
